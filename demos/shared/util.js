...
export function drawResults(ctx, faces, triangulateMesh, boundingBox) {
  faces.forEach((face) => {
    ...
    // const box = face.box;
    // const center = [(box.xMax + box.xMin) / 2, (box.yMax + box.yMin) / 2];
    // const center = [face.keypoints[159].x, face.keypoints[159].y];

    // console.log(face.keypoints[168]);

    // console.log(center);

    // const cheek1 = face.keypoints[127]; // cheek
    // const cheek2 = face.keypoints[356]; // cheek
    // const nose = face.keypoints[6]; // nose
    ...
    // const averageVector = [
    //   (leftVector[0] + rightVector[0]) / 2,
    //   (leftVector[1] + rightVector[1]) / 2,
    //   (leftVector[2] + rightVector[2]) / 2,
    // ];

    // ctx.strokeStyle = "red"; // Set color for the vector
    // ctx.lineWidth = 5; // Set line width for the vector
    ...
    // drawPath(ctx, [
    //   center,
    //   [center[0] + averageVector[0] * 1, center[1] + averageVector[1] * 1],
    // ]);
  });
}
 ...
 */
export function getDirectionVectors(meshPoints) {
  // These points lie *almost* on same line in uv coordinates
  const cheek1 = meshPoints[127]; // cheek
  const cheek2 = meshPoints[356]; // cheek
  const nose = meshPoints[6]; // nose

  const leftVector = [nose.x - cheek1.x, nose.y - cheek1.y, nose.z - cheek1.z];
  const rightVector = [nose.x - cheek2.x, nose.y - cheek2.y, nose.z - cheek2.z];
